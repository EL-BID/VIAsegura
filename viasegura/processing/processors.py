...
class Images_Labeler:
    def __init__(
        self,
        processor_config_file="config.json",
        ...
        if gpus:
            try:
                tf.config.experimental.set_virtual_device_configuration(
                    gpus[0],
                    [
                        tf.config.experimental.VirtualDeviceConfiguration(memory_limit=6 * memory_unit),
